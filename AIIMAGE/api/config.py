...
class Settings(BaseSettings):
    ...
    # Product image quality gate (rule-based, no external AI)
    PRODUCT_IMAGE_QUALITY_ENABLED: bool = True
    PRODUCT_IMAGE_PRIMARY_MIN_WIDTH: int = Field(default=1000, ge=100, le=10000)
    PRODUCT_IMAGE_PRIMARY_MIN_HEIGHT: int = Field(default=1000, ge=100, le=10000)
    PRODUCT_IMAGE_ADDITIONAL_MIN_WIDTH: int = Field(default=600, ge=100, le=10000)
    PRODUCT_IMAGE_ADDITIONAL_MIN_HEIGHT: int = Field(default=600, ge=100, le=10000)
    PRODUCT_IMAGE_PRIMARY_MIN_MEGAPIXELS: float = Field(default=1.0, ge=0.01, le=100.0)
    PRODUCT_IMAGE_ADDITIONAL_MIN_MEGAPIXELS: float = Field(default=0.36, ge=0.01, le=100.0)
    PRODUCT_IMAGE_PRIMARY_MIN_ASPECT_RATIO: float = Field(default=0.75, gt=0, le=10)
    PRODUCT_IMAGE_PRIMARY_MAX_ASPECT_RATIO: float = Field(default=1.33, gt=0, le=10)
    PRODUCT_IMAGE_ADDITIONAL_MIN_ASPECT_RATIO: float = Field(default=0.40, gt=0, le=10)
    PRODUCT_IMAGE_ADDITIONAL_MAX_ASPECT_RATIO: float = Field(default=2.50, gt=0, le=10)
    PRODUCT_IMAGE_PRIMARY_MIN_SHARPNESS: float = Field(default=20.0, ge=0)
    PRODUCT_IMAGE_ADDITIONAL_MIN_SHARPNESS: float = Field(default=12.0, ge=0)
    PRODUCT_IMAGE_MIN_BRIGHTNESS: float = Field(default=45.0, ge=0, le=255)
    PRODUCT_IMAGE_MAX_BRIGHTNESS: float = Field(default=245.0, ge=0, le=255)
    PRODUCT_IMAGE_PRIMARY_MIN_BACKGROUND_WHITENESS: float = Field(default=60.0, ge=0, le=100)
    PRODUCT_IMAGE_WHITE_PIXEL_THRESHOLD: int = Field(default=235, ge=0, le=255)
    PRODUCT_IMAGE_BACKGROUND_BORDER_PERCENT: int = Field(default=10, ge=2, le=30)

    # Optional AI-assisted product image improvement.
    # Keep OPENROUTER_API_KEY on the backend only; never expose it to Next.js.
    PRODUCT_IMAGE_AI_ENABLED: bool = False
    OPENROUTER_API_KEY: str | None = None
    OPENROUTER_IMAGE_API_URL: str = "https://openrouter.ai/api/v1/images"
    PRODUCT_IMAGE_AI_MODEL: str = "openai/gpt-image-1"
    PRODUCT_IMAGE_AI_TIMEOUT_SECONDS: int = Field(default=120, ge=15, le=300)
    PRODUCT_IMAGE_AI_RESOLUTION: str = "1K"
    PRODUCT_IMAGE_AI_QUALITY: Literal["auto", "low", "medium", "high"] = "high"
    PRODUCT_IMAGE_AI_MAX_SOURCE_MB: int = Field(default=8, ge=1, le=20)
