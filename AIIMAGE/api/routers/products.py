...
from api.services.product_ai_image_service import improve_product_image
from api.services.product_image_service import (
    MAX_PRODUCT_IMAGES,
    delete_product_image_files,
    ensure_stored_product_image_primary_quality,
    store_product_image,
)
...
def submit_product_for_review(
    ...
    if primary_image:
        ensure_stored_product_image_primary_quality(primary_image.storage_key)
    else:
        first = db.query(ProductImage).filter(ProductImage.product_id == product.id).order_by(ProductImage.display_order.asc()).first()
        if first:
            ensure_stored_product_image_primary_quality(first.storage_key)
...
@router.post("/{product_id}/images/ai-improve")
async def improve_product_image_with_ai(
    product_id: UUID,
    file: UploadFile = File(..., description="Original JPEG, PNG or WEBP product image"),
    is_primary: bool = Form(default=False),
    db: Session = Depends(get_db),
    current_user: User = Depends(require_permission(PermissionCode.seller_product_images_manage.value)),
):
    """Generate a temporary AI-improved preview from a seller's real product photo.

    This endpoint does not attach the generated image to the product. The normal
    Xerin image-quality gate evaluates the AI result and the seller must explicitly
    accept/upload it in the later UI flow.
    """
    seller = get_my_seller(db, current_user)
    product = get_seller_product(db, product_id, seller.id)
    _ensure_editable(product)
    ...
    raw = await file.read()
    improved = improve_product_image(raw, is_primary=is_primary)
    quality_json = json.dumps(improved.quality, separators=(",", ":"))
    return Response(
        content=improved.image_bytes,
        media_type=improved.mime_type,
        headers={
            "X-Xerin-AI-Model": improved.model,
            "X-Xerin-Image-Width": str(improved.width),
            "X-Xerin-Image-Height": str(improved.height),
            "X-Xerin-Quality-Passed": "true" if improved.quality["passed"] else "false",
            "X-Xerin-Quality-Score": str(improved.quality["score"]),
            "X-Xerin-Quality": base64.urlsafe_b64encode(quality_json.encode("utf-8")).decode("ascii"),
...
async def upload_product_images(
    ...
    try:
        for index, file in enumerate(files):
            ...
            stored = await store_product_image(
                file,
                seller_id=seller.id,
                product_id=product.id,
...
def add_product_image_url(
    ...
    if db.query(ProductImage).filter(ProductImage.product_id == product.id).count() >= MAX_PRODUCT_IMAGES:
        raise HTTPException(status_code=400, detail=f"Maximum {MAX_PRODUCT_IMAGES} images allowed per product")
    if settings.PRODUCT_IMAGE_QUALITY_ENABLED:
        raise HTTPException(
            status_code=409,
            detail={
                "code": "PRODUCT_IMAGE_UPLOAD_REQUIRED",
                "message": "Direct image URLs are disabled while the product image quality gate is enabled. Upload the image file instead.",
            },
        )
    image = ProductImage(
        product_id=product.id,
...
def update_product_image(
    ...
    if update_data.pop("is_primary", False):
        ensure_stored_product_image_primary_quality(image.storage_key)
        _make_primary(db, product.id, image.id)
    for key, value in update_data.items():
...
def delete_product_image(
    ...
    if was_primary:
        ...
        for next_image in candidates:
            try:
                ensure_stored_product_image_primary_quality(next_image.storage_key)
