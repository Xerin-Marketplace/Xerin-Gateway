...
def release_escrow_hold(
    ...
    if not hold:
        raise HTTPException(404, "Escrow hold not found")
    if hold.status not in {"held", "release_pending"}:
        raise HTTPException(409, f"Escrow hold cannot be released from status {hold.status}")

    available = Decimal(hold.gross_amount) - Decimal(hold.refunded_amount) - Decimal(hold.released_amount)
    requested = Decimal(data.amount) if data.amount is not None else available

    settings = _settings(db)
    if requested != available and not settings.allow_partial_release:
        raise HTTPException(409, "Partial escrow release is disabled")
    if requested <= 0 or requested > available:
        raise HTTPException(422, "Release amount exceeds the remaining held amount")

    hold.released_amount = Decimal(hold.released_amount) + requested
    remaining = Decimal(hold.gross_amount) - Decimal(hold.refunded_amount) - Decimal(hold.released_amount)
