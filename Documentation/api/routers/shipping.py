...
from api.models import (
    Address, LogisticsCompany, MarketplaceSettings, Order, Shipment, ShipmentStatus,
    ShipmentTrackingEvent, ShippingMethod, ShippingRate, ShippingZone, User,
)
...
def quote_shipping(
    ...
    is_tanzania = country.lower() in {"tanzania", "united republic of tanzania", "tz"}
    requested_scope = "local" if is_tanzania else "international"

    if not is_tanzania:
        settings_row = db.query(MarketplaceSettings).first()
        if not settings_row or not settings_row.international_delivery_allowed:
            raise HTTPException(
                status_code=409,
    ...
    matching_zone_ids = []
    for zone in zones:
        zone_scope = zone.scope.value if hasattr(zone.scope, "value") else str(zone.scope)
        if requested_scope == "local" and zone_scope not in {"local", "both"}:
            continue
        if requested_scope == "international" and zone_scope not in {"international", "both"}:
            continue
    ...
    rates = (
        ...
        .options(
            joinedload(ShippingRate.method).joinedload(ShippingMethod.logistics_company)
        )
        .filter(
    ...
    for rate in rates:
        ...
        method_scope = method.scope.value if hasattr(method.scope, "value") else str(method.scope)
        if requested_scope == "local" and method_scope not in {"local", "both"}:
            continue
        if requested_scope == "international" and method_scope not in {"international", "both"}:
            continue

        company = method.logistics_company
        if company:
            company_status = (
                company.status.value if hasattr(company.status, "value") else str(company.status)
            )
            if company_status != "active":
                continue

        if rate.min_weight_kg is not None and data.weight_kg < rate.min_weight_kg:
            continue
        if rate.max_weight_kg is not None and data.weight_kg > rate.max_weight_kg:
            continue

        if (
            rate.free_shipping_threshold is not None
            and data.subtotal >= rate.free_shipping_threshold
        ):
            amount = Decimal("0.00")
        ...
        options.append(
            ShippingQuoteOption(
                ...
                logistics_company_name=company.name if company else method.carrier_name,
                method_name=method.name,
                carrier_name=method.carrier_name or (company.name if company else None),
                scope=method.scope,
                supports_cod=method.supports_cod and bool(company.supports_cod if company else True),
                supports_tracking=method.supports_tracking and bool(company.supports_tracking if company else True),
    ...
    return sorted(options, key=lambda item: (item.currency, item.amount))
