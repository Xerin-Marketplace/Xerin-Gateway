...
from api.services.zenopay_service import ZenoPayAPIError, ZenoPayClient, ZenoPayConfigurationError
...
def initiate_payment(data: PaymentInitiateRequest, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    if payment.provider == "zenopay":
        ...
        try:
            ...
            _record_transaction(db, payment, "provider_rejected", PaymentStatus.failed.value,
                                order.total, {"provider": "zenopay", "code": "invalid_payment_request",
                                              "message": str(exc), "retryable": False})
            _commit(db)
            raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_CONTENT,
                                detail=_payment_error_detail(code="INVALID_PAYMENT_REQUEST", message=str(exc),
                                                             payment=payment, order=order, retryable=False)) from exc
        except ZenoPayConfigurationError as exc:
            payment.status = PaymentStatus.failed
            payment.failure_reason = str(exc)
            _record_transaction(db, payment, "provider_configuration_error", PaymentStatus.failed.value,
                                order.total, {"provider": "zenopay", "message": str(exc), "retryable": False})
            _commit(db)
            raise HTTPException(status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
                                detail=_payment_error_detail(code="PAYMENT_PROVIDER_CONFIGURATION_ERROR",
                                                             message=str(exc), payment=payment, order=order,
                                                             retryable=False)) from exc
        except ZenoPayAPIError as exc:
            payment.status = PaymentStatus.failed
            payment.failure_reason = str(exc)
            _record_transaction(db, payment, "provider_error", PaymentStatus.failed.value,
        ...
        if not result.accepted:
            payment.status = PaymentStatus.failed
            payment.failure_reason = result.message or "ZenoPay rejected the payment request"
            ...
            raise HTTPException(status_code=status.HTTP_502_BAD_GATEWAY,
                                detail=_payment_error_detail(code="PAYMENT_PROVIDER_REJECTED",
                                                             message=payment.failure_reason, payment=payment,
                                                             order=order, retryable=False))

        payment.status = PaymentStatus.processing
        payment.provider_transaction_id = result.provider_reference
        payment.provider_response = {**result.raw, "external_order_id": result.external_order_id,
                                     "message": result.message, "mno": data.provider}
        _record_transaction(db, payment, "provider_request", PaymentStatus.processing.value,
                            order.total, payment.provider_response)
...
def _verify_and_apply_zenopay_status(payment: Payment, db: Session) -> Payment:
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
                            detail="ZenoPay verification is not configured") from exc
    except ZenoPayAPIError as exc:
        http_status = exc.status_code if exc.status_code in {502, 503, 504} else status.HTTP_502_BAD_GATEWAY
        raise HTTPException(status_code=http_status,
                            detail={"code": "PAYMENT_STATUS_VERIFICATION_FAILED",
                                    "provider": "zenopay", "message": str(exc),
    ...
    if result.amount is not None and result.amount != Decimal(payment.amount):
        rejection_key = _callback_idempotency_key(
            "zenopay", payment.id, result.provider_reference or str(payment.id), "amount_mismatch"
        )
        if not _callback_already_processed(db, rejection_key):
            _record_transaction(
                db, payment, "callback_rejected", "amount_mismatch", payment.amount,
                {"provider": "zenopay", "expected_amount": str(payment.amount),
                 "provider_amount": str(result.amount), "external_order_id": result.external_order_id},
    ...
    transaction_id = result.provider_reference or payment.provider_transaction_id or str(payment.id)
...
def zenopay_webhook(payload: ZenoPayWebhookRequest, db: Session = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                            detail="Invalid ZenoPay order_id") from exc
    ...
    return {"received": True, "payment_id": str(verified.id), "status": verified.status.value}
