...
def _product_item(db: Session, row: WishlistProduct) -> WishlistProductItemResponse:
    product = row.product
    store = (product.seller.stores[0] if product.seller and product.seller.stores else None)
...
def list_wishlist_products(
    ...
    query = db.query(WishlistProduct).options(
        ...
        joinedload(WishlistProduct.product).joinedload(Product.seller).joinedload(Seller.stores),
