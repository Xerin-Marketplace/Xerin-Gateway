...
def test_callback_signature_verification_valid_and_tampered():
    ...
    utilityref = "payment-uuid"
    externalreference = "azam-ref"
    transactionstatus = "success"
    operator = "Airtel"

    signed_data = AzamPayClient.callback_signed_data(
        utilityref,
        externalreference,
        transactionstatus,
        operator,
    ...
    assert AzamPayClient.verify_callback_signature_with_pem(
    ...
    assert not AzamPayClient.verify_callback_signature_with_pem(
        utility_ref=utilityref,
        external_reference=externalreference,
...
def test_callback_signature_rejects_invalid_base64():
    private_key = rsa.generate_private_key(public_exponent=65537, key_size=2048)
    public_pem = (
        private_key.public_key()
        .public_bytes(
...
def test_callback_requires_signature_field(client):
    payload = {
        "message": "ok",
        "user": "merchant",
        "password": "hidden",
        ...
        "utilityref": "00000000-0000-0000-0000-000000000001",
        "amount": "1000",
        "transid": "tx",
        "msisdn": "255700000000",
        "mnoreference": "mno",
    }
    response = client.post("/api/v1/payments/azampay/callback", json=payload)
    assert response.status_code == 401
    assert response.json()["detail"] == "AzamPay callback signature is required"
