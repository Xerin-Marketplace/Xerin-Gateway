import re
from api.countries import require_canonical_country
...
import enum
from api.enums import DayOfWeek, StoreStatus, StoreScope, ShippingRateType
...
class LoginRequest(BaseModel):
    ...
    password: str
    # Persistent browser/device identifier generated by the frontend.
...
class TokenResponse(BaseModel):
    ...
    user: dict | None = None
    security_notice: dict | None = None
...
class OrderResponse(BaseModel):
    ...
    cancellation_email_sent_at: Optional[datetime] = None
...
class LogisticsCredentialsEmailResponse(BaseModel):
    company_id: UUID
    administrator_user_id: UUID
    email: EmailStr
    credentials_email_sent: bool = True
    message: str
...
class ShippingZoneCreate(BaseModel):
    ...
    def normalise_country(cls, value: str) -> str:
        return require_canonical_country(value)
    ...
    def validate_coverage(self):
        if not any(
            (
                self.supports_domestic_delivery,
                self.supports_cross_border_inbound,
                self.supports_cross_border_outbound,
            )
        ):
            raise ValueError(
                "Enable at least one route capability: domestic, cross-border inbound, or cross-border outbound"
            )
...
class ShippingZoneUpdate(BaseModel):
    ...
    @field_validator("country")
    @classmethod
    def normalise_optional_country(cls, value: Optional[str]) -> Optional[str]:
        if value is None:
            return None
        return require_canonical_country(value)
...
class IneligibleLogisticsCompanyReason(BaseModel):
    logistics_company_id: UUID
    name: str
    code: str
    reason_codes: list[str] = Field(default_factory=list)
    reasons: list[str] = Field(default_factory=list)
    uncovered_sellers: list[str] = Field(default_factory=list)
...
class PaginatedEligibleLogisticsCompanyResponse(BaseModel):
    ...
    excluded_companies: list[IneligibleLogisticsCompanyReason] = Field(default_factory=list)
...
class SellerPricingPreviewResponse(BaseModel):
    ...
    seller_sale_net_amount: Optional[Decimal] = None
