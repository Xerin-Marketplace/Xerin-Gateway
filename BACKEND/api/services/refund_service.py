...
def create_refund_request(db:Session,*,order:Order,user_id,data):
    ...
    refund=Refund(order_id=order.id,requested_by_id=user_id,status=RefundStatus.requested,reason=data.reason,reason_details=data.reason_details,currency=order.currency,idempotency_key=data.idempotency_key,items_amount=0,shipping_amount=money(order.shipping_amount) if data.refund_shipping else 0,tax_amount=money(order.tax_amount) if data.refund_tax else 0,total_amount=0)
    ...
    for item_id,req in requested.items():
        ...
        already=db.query(RefundItem).join(Refund).filter(RefundItem.order_item_id==item.id,Refund.status.in_([RefundStatus.requested,RefundStatus.under_review,RefundStatus.approved,RefundStatus.processing,RefundStatus.completed])).with_entities(RefundItem.quantity).all()
        remaining=item.quantity-sum(x[0] for x in already)
        if req.quantity>remaining: raise ValueError(f"Refund quantity exceeds refundable quantity for {item.product_name}")
        amount=money(Decimal(item.unit_price)*req.quantity);items_total+=amount
        db.add(RefundItem(refund_id=refund.id,order_item_id=item.id,seller_id=item.seller_id,quantity=req.quantity,unit_amount=money(item.unit_price),refund_amount=amount,restock=req.restock))
...
def complete_refund(db:Session,refund:Refund,*,user_id=None):
    for ri in refund.items:
        ...
        commission_reversal=money(Decimal(commission.commission_amount)*ratio) if commission else Decimal("0")
        seller_reversal=money(Decimal(ri.refund_amount)-commission_reversal)
