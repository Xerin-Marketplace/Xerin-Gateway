...
class AzamPayClient:
    ...
    _public_key_expires_at: float = 0
    ...
    def get_public_key_pem(self, *, force_refresh: bool = False) -> str:
        """Fetch and cache AzamPay's callback RSA public key.

        AzamPay recommends periodic caching (for example 24 hours) and a single
        forced refresh when signature verification fails.
        """
        now = time.time()
        if (
            not force_refresh
            and self.__class__._public_key_pem
            and now < self.__class__._public_key_expires_at
        ):
            return self.__class__._public_key_pem

        response = self._get(settings.AZAMPAY_PUBLIC_KEY_PATH)
        data = self._json_response(response)
        if not response.ok or data.get("success") is False:
            raise AzamPayAPIError(
                data.get("message") or "AzamPay public-key request failed",
                status_code=response.status_code,
                payload=data,
                retryable=response.status_code >= 500,
            )
        ...
        if not isinstance(pem, str) or "BEGIN PUBLIC KEY" not in pem:
            raise AzamPayAPIError(
                "AzamPay public-key response did not contain a valid PEM public key",
                status_code=response.status_code,
                payload=data,
            )

        self.__class__._public_key_pem = pem
        self.__class__._public_key_expires_at = (
            time.time() + settings.AZAMPAY_PUBLIC_KEY_CACHE_SECONDS
        )
        return pem

    @staticmethod
    def callback_signed_data(
        utility_ref: str,
        ...
        return (
            f"{utility_ref}{external_reference}{transaction_status}{operator_name}"
        ).encode("utf-8")

    @staticmethod
    def verify_callback_signature_with_pem(
        *,
        utility_ref: str,
        external_reference: str,
        transaction_status: str,
        ...
        try:
            ...
            public_key.verify(
                signature_bytes,
                AzamPayClient.callback_signed_data(
                    utility_ref,
                    external_reference,
                    transaction_status,
                    operator_name,
    ...
    def verify_callback_signature(
        ...
        *,
        ...
        """Verify callback signature; refresh the key once on failure."""
        if not signature_b64:
            return False

        pem = self.get_public_key_pem()
        if self.verify_callback_signature_with_pem(
        ...
        # AzamPay recommends one public-key refresh and retry if verification
        # fails, to handle key rotation without accepting an invalid callback.
        pem = self.get_public_key_pem(force_refresh=True)
        return self.verify_callback_signature_with_pem(
            utility_ref=utility_ref,
            external_reference=external_reference,
            transaction_status=transaction_status,
            operator_name=operator_name,
