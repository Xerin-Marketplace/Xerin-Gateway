...
def get_usable_checkout_delivery_quote(
    ...
    *,
    ...
    delivery_quote_id: UUID | None = None,
    user_id: UUID,
    shipping_address_id: UUID,
    delivery_mode: str,
    lock: bool = False,
) -> CheckoutDeliveryQuote:
    """Return a usable CheckoutDeliveryQuote.

    Accepts either `quote_id` (legacy) or `delivery_quote_id` (caller uses this name).
    """
    final_id = quote_id or delivery_quote_id
    if final_id is None:
        raise CheckoutDeliveryQuoteError(
            "Delivery quote id is required.",
            code="checkout_delivery_quote_id_required",
            status_code=400,
        )

    query = db.query(CheckoutDeliveryQuote).filter(
        CheckoutDeliveryQuote.id == final_id,
