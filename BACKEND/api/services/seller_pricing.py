...
def calculate_marketplace_price(
    ...
    *,
    ...
    if rule and rule.rule_type == CommissionRuleType.fixed:
        commission = min(base, money(rate))
    else:
        commission = money(base * rate / Decimal("100"))

    # Seller-funded commission model:
    # the seller-entered listing price is exactly what the customer pays.
    # Xerin commission is deducted later from the seller settlement.
    seller_net = money(max(Decimal("0.00"), base - commission))
