...
def build_order_workflow(db: Session, order: Order) -> dict[str, Any]:
    ...
    seller_order_by_origin = {(row.seller_id, row.store_id): row for row in seller_orders}
    shipment_flows: list[dict[str, Any]] = []
    delivered_count = 0
    for shipment in shipments:
        seller_order = seller_order_by_origin.get((shipment.seller_id, shipment.store_id))
...
def reconcile_order_workflow(db: Session, order: Order, *, actor_id: Any) -> dict[str, Any]:
    ...
    seller_orders = {
        (row.seller_id, row.store_id): row
    ...
    for shipment in shipments:
        ...
        seller_order = seller_orders.get((shipment.seller_id, shipment.store_id))
