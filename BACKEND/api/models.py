...
class Session(Base):
    ...
    is_recognized = Column(Boolean, nullable=False, default=True, server_default="true")
...
class SellerProfile(Base):
    ...
    business_district = Column(String(100), nullable=True)
    business_ward = Column(String(100), nullable=True)
...
class Order(Base):
    ...
    # Pending online-checkout lifecycle. New checkout orders receive a deadline;
    # COD/paid orders naturally leave pending status before the worker can expire them.
    payment_due_at = Column(DateTime(timezone=True), nullable=True, index=True)
    cancelled_at = Column(DateTime(timezone=True), nullable=True, index=True)
    cancellation_reason = Column(String(120), nullable=True, index=True)
...
class OrderItem(Base):
    ...
    # Immutable fulfillment origin captured when the order is created.
...
class SellerOrder(Base):
    ...
    __table_args__ = (
        UniqueConstraint("order_id", "seller_id", "store_id", name="uq_seller_order_order_seller_store"),
        CheckConstraint(
...
class Shipment(Base):
    ...
    __table_args__ = (
        UniqueConstraint("order_id", "seller_id", "store_id", name="uq_shipment_order_seller_store"),
    )
...
class Store(Base):
    ...
    store_scope = Column(
        Enum(StoreScope, values_callable=lambda enum_cls: [item.value for item in enum_cls]),
        nullable=False,
        default=StoreScope.local,
        server_default="local",
        index=True,
    )
