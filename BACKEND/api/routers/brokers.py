...
from api.services.product_image_service import MAX_PRODUCT_IMAGES, store_product_image, delete_product_image_files, ensure_stored_product_image_primary_quality
...
def delete_broker_product_image(product_id: uuid.UUID, image_id: uuid.UUID, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    if was_primary:
        candidates=db.query(ProductImage).filter(ProductImage.product_id==product.id).order_by(ProductImage.display_order.asc()).all()
...
def publish_broker_product(product_id: uuid.UUID, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    primary_image=db.query(ProductImage).filter(ProductImage.product_id==product.id,ProductImage.is_primary.is_(True)).first()
    if not primary_image:
        primary_image=db.query(ProductImage).filter(ProductImage.product_id==product.id).order_by(ProductImage.display_order.asc()).first()
        if primary_image:
