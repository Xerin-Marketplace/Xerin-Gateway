...
def preview_commission_price(
    ...
    if rule is None:
        ...
        commission = min(base, _money(rate))
    else:
        rate = Decimal(rule.rate)
    ...
    return {
        ...
        # Seller-funded commission: customer pays the seller-listed amount.
        "customer_price": base,
        "seller_receivable_before_other_adjustments": _money(
            max(Decimal("0.00"), base - commission)
        ),
