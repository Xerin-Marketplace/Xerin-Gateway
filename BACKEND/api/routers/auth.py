...
def _is_staff_account(db: Session, user: User) -> bool:
    """Treat Admin/super-admin and custom users with Admin permissions as staff."""
    roles = {name.lower() for name in get_user_role_names(user)}
    if roles & {"admin", "super_admin"} or any("staff" in name for name in roles):
        return True
    try:
    ...
    return any(
        ...
        or code.startswith("admin:")
        or code.startswith("admin-")
        for code in permissions
    )


def _approximate_location(request: Request) -> str | None:
    """Use reverse-proxy geo headers when available; never claim precise GPS."""
    city = (
...
def _fallback_device_name(user_agent: str | None) -> str:
...
def _create_authenticated_session(
    ...
    *,
    ...
    """Create a session, bind access token to it, and return any staff new-device alert."""
    now = datetime.now(timezone.utc)
    ip = _client_ip(request) if request is not None else None
    user_agent = request.headers.get("user-agent") if request is not None else None
    location = _approximate_location(request) if request is not None else None
    cleaned_device_id = (device_id or "").strip() or None
    cleaned_device_name = (device_name or "").strip() or _fallback_device_name(user_agent)

    active_sessions = (
        db.query(UserSession)
        .filter(UserSession.user_id == user.id, UserSession.expires_at > now)
    ...
    different_sessions = [
        ...
        if cleaned_device_id and item.device_id and item.device_id != cleaned_device_id
    ]
    is_staff = _is_staff_account(db, user)
    new_device_alert = bool(is_staff and cleaned_device_id and different_sessions)
    ...
    session = UserSession(
        ...
        is_recognized=not new_device_alert,
        last_seen_at=now,
    )
    db.add(session)
    db.flush()
    ...
    notice = None
    if new_device_alert:
        previous = different_sessions[0]
        notice = {
            "code": "STAFF_NEW_DEVICE_LOGIN",
            "message": "This staff account is already signed in on another device.",
            ...
            "existing_session": {
                ...
                "device_name": previous.device_name or "Unknown device",
                "ip_address": previous.ip_address,
                "approximate_location": previous.approximate_location,
                "last_seen_at": previous.last_seen_at.isoformat() if previous.last_seen_at else None,
...
def list_active_sessions(
    ...
    return {
        "items": [
            {
                "id": str(item.id),
                "device_name": item.device_name or "Unknown device",
                "ip_address": item.ip_address,
                "approximate_location": item.approximate_location,
                "is_recognized": bool(item.is_recognized),
...
@router.post("/sessions/current/recognize")
def recognize_current_session(
    request: Request,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    ...
    try:
        ...
        sid = None
    if not sid:
        raise HTTPException(status_code=400, detail="Current session identifier is unavailable")
    session = (
        db.query(UserSession)
        .filter(UserSession.id == UUID(str(sid)), UserSession.user_id == current_user.id)
        .first()
    )
    if not session:
        raise HTTPException(status_code=404, detail="Session not found")
    session.is_recognized = True
    session.last_seen_at = datetime.now(timezone.utc)
    db.commit()
    return {"message": "Device recognized"}
...
def revoke_other_sessions(
    ...
    if sid:
        query = query.filter(UserSession.id != UUID(str(sid)))
    count = query.delete(synchronize_session=False)
    db.commit()
    return {"message": "Other sessions revoked", "revoked_count": count}


@router.post("/security/unrecognized-login")
def report_unrecognized_login(
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    """Emergency response: invalidate every refresh/access session for this account."""
    count = (
        db.query(UserSession)
    ...
    db.commit()
    logger.warning(
        "SECURITY: user %s reported an unrecognized staff login; revoked %s sessions",
        current_user.id,
        count,
    )
    return {
        "message": "All active sessions have been revoked. Reset your password before signing in again.",
        "revoked_count": count,
        "reset_required": True,
    }
