...
from api.models import Campaign, CampaignPromotion, Promotion, PromotionRule, Product, Category, Seller, User
from api.permissions import require_permission
from api.schemas import (
    CampaignCreate, CampaignResponse, PromotionApplyRequest, PromotionApplyResponse,
    PromotionCreate, PromotionResponse, PromotionUpdate, PaginatedPromotionResponse,
)
...
@router.get("/seller/promotions", response_model=PaginatedPromotionResponse)
def seller_promotions(
    ...
    current_user: User = Depends(require_permission(PermissionCode.promotions_read.value)),
):
    seller = _seller(current_user)
    query = db.query(Promotion).filter(Promotion.seller_id == seller.id)
    if active is not None:
        query = query.filter(Promotion.is_active.is_(active))
    ...
    total = query.count()
    rows = query.order_by(Promotion.created_at.desc()).offset((page - 1) * page_size).limit(page_size).all()
    return {
...
def create_seller_promotion(data: PromotionCreate, db: Session = Depends(get_db), current_user: User = Depends(require_permission(PermissionCode.promotions_create.value))):
    seller = _seller(current_user)
    # Seller-funded promotions may target only this seller's products.
    for rule in data.rules:
        if rule.product_id:
            owned = db.query(Product.id).filter(
                Product.id == rule.product_id,
                Product.seller_id == seller.id,
            ).first()
            if not owned:
                raise HTTPException(422, "Promotion contains a product not owned by this seller")

    promotion = Promotion(
        **data.model_dump(exclude={"rules"}),
        seller_id=seller.id,
