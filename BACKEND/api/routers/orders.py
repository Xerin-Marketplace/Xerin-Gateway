...
def download_customer_payment_receipt(
    order_id: UUID,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    ...
    if completed_payment is None:
        raise HTTPException(
            ...
            detail={
                ...
                "message": "A payment receipt is available only after payment is verified as completed.",
            },
        )

    pdf = build_payment_receipt_pdf(order, completed_payment)
    paid_at = completed_payment.paid_at or completed_payment.created_at
    filename = f"Xerin-Receipt-{paid_at:%Y%m%d}-{str(completed_payment.id)[:8].upper()}.pdf"
    return StreamingResponse(
        iter([pdf]),
        media_type="application/pdf",
        headers={
